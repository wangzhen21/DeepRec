...
from models.rating_prediction.NeuMF_dire import NeuMF_dire
...
def parse_args():
    ...
    parser.add_argument('--model', choices=['MF', 'NNMF', 'NRR', 'I-AutoRec', 'U-AutoRec', 'FM', 'NFM', 'AFM','NeuMF_dire'], default='NeuMF_dire')
...
if __name__ == '__main__':
    ...
    train_data, test_data, n_user, n_item ,n_dire= load_data_rating_menu_dire_neg_pos(trainpath="../data/ml1m/train_1m_ratings.dat", testpath="../data/ml1m/test_1m_ratings.dat",
    ...
    with tf.Session(config=config) as sess:
        model = None
        # Model selection
        if args.model == "NeuMF_dire":
            args1 = utils.parse_args()
            model = NeuMF_dire(args1, n_user, n_item,47)
        if args.model == "MF":

...
from models.rating_prediction.mf_dire import MF_dire
from utils.load_data.load_data_rating import load_data_rating_dir
...
if __name__ == '__main__':
    ...
    train_data, test_data, n_user, n_item,dire_num = load_data_rating_dir(path="../Data/ml1m/ratings_date_dire_t_with_dir_num.dat",
                                                             header=['user_id', 'item_id', 'rating', 't','this_dire_time','whole_dire_time','dire_index','dire_name'],
                                                             test_size=0.1, sep="\t")
    ...
    with tf.Session(config=config) as sess:
        ...
        # Model selection
        if args.model == "MF":
            model = MF_dire(sess, n_user, n_item, dire_num,batch_size=batch_size)
        if args.model == "NNMF":

...
from models.rating_prediction.MLP_dire import MLP_dire
...
def parse_args():
    ...
    parser.add_argument('--model', choices=['MF', 'NNMF', 'NRR', 'I-AutoRec', 'U-AutoRec', 'FM', 'NFM', 'AFM','MLP_dire'], default='MLP_dire')
...
if __name__ == '__main__':
    ...
    with tf.Session(config=config) as sess:
        model = None
        # Model selection
        if args.model == "MLP_dire":
            args1 = utils.parse_args()
            model = MLP_dire(args1, n_user, n_item,47)
        if args.model == "MF":

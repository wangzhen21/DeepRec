...
class MLP_dire_all():
    ...
    def build_core_model(self, user_indices, item_indices,dire_pos_indices,dire_neg_indices):
        ...
        emb_dire_all = tf.Variable(tf.truncated_normal([self.dire_num, 2],
        ...
        all_dire_feature = tf.nn.embedding_lookup(emb_dire_all, dire_pos_indices, name='dire_all_feature')
        hidden_layers = [tf.concat([user_feature, item_feature, all_dire_feature], 1)]

        model_params = [emb_user, emb_item,all_dire_feature]
    ...
    def build_network(self, user_indices=None, item_indices=None,all_dire_indices = None):
        ...
        if not all_dire_indices:
            all_dire_indices = tf.placeholder(tf.int32, [None])
        self.all_dire_indices = all_dire_indices

        self.ratings = tf.placeholder(tf.float32, [None])

        model_vector, model_len, model_params = self.build_core_model(user_indices,item_indices,all_dire_indices)
    ...
    def predict(self,user_id,item_id,dire_all):
        return self.sess.run([self.output], feed_dict={self.user_indices : user_id, self.item_indices : item_id,self.all_dire_indices:dire_all})[0]
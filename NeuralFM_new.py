...
class NeuralFM(BaseEstimator, TransformerMixin):
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():  # , tf.device('/cpu:0'):
            ...
            self.train_features_add = tf.placeholder(tf.int32, shape=[None, 2], name="add_f")
            self.train_labels = tf.placeholder(tf.float32, shape=[None, 1])  # None * 1 #[None, 1] 和 [None]区别
